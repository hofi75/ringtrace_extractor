...
/* original 
typedef struct _ring_trace
{
   char                 eyec[8];
   //                    12345678
#define RING_TRACE_EYEC "%RTRACE%"
   char                 id[16];       
   char                 time[__XDAT_SIZE];
   struct _ring_trace   *nxt;
   void                 *addr;
   char                 *dump;
   char                 *text;
   int                  size;
...
int main( int argc, char *argv[])
{
    ...
    for( prev_id = 0, p = addr, i = 0, entries = 0; i < length; i++, p++, prev_id = id)
    {
        ...
        if ( !isdigit( ( int) rt64le->id[0])) continue;
...
{
    printf( "%8.8s-0000/%.16lX - %s - %s\n", rt->id, rt->thread, rt->time, rt->text);
...
int print_rt_dump( ring_trace_t *rt)
{
    ...
    // dump text
    printf( "%8.8s-0000/%.16lX - %s - DUMP(%s) (@%p, size=%d(0x%X))\n", 
        rt->id, rt->thread, rt->time, rt->text, rt->addr, rt->dump_len, rt->dump_len);

    int lnr = 1;
    sprintf( s1, "%8.8s-%.4X/%.16lX - \t\t\t%p-%.5X: ", rt->id, lnr++, rt->thread, orig_addr, 0);
    s2[0] = '\0';
    s4[1] = '\0';
    ...
    for ( i = 0; i < rt->dump_len; i++)
    {
        ...
        {
            printf( "%s  >%s<\n", s1, s2);
            sprintf( s1, "%8.8s-%.4X/%.16lX - \t\t\t%p-%.5X: ", rt->id, lnr++, rt->thread, orig_addr, i + 1);
            s2[0] = '\0';
        }
    }
